...
export const message = async (data: any,
  ...
  if (!room) {
    logger.warn(`Room not found with name=${data.name}, text=${data.text}, socket id=${socket.id}, timestamp=${Date.now()}.
    It's likely that the chat already ended and this message is coming up later.`);
  } else if (room!.endChatTime >= Date.now()) {
    ...
    if (sendingUser?.canSend) {
      ...
      logger.warn(`User cannot send message in ${id}`);
    }
  } else {
    logger.warn(`Unable to send message due to time limit in ${id}`);
  }
}
